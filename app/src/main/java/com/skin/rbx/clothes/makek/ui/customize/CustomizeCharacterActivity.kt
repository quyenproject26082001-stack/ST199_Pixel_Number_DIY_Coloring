...
import com.skin.rbx.clothes.makek.core.extensions.dLog
import com.skin.rbx.clothes.makek.core.extensions.eLog
...
import com.skin.rbx.clothes.makek.core.extensions.logEvent
...
import com.skin.rbx.clothes.makek.core.extensions.startIntentLeftToRight
...
import com.skin.rbx.clothes.makek.data.model.custom.ItemNavCustomModel
import com.skin.rbx.clothes.makek.databinding.ActivityCustomizeBinding
...
import com.skin.rbx.clothes.makek.core.extensions.tapAndHold
...
class CustomizeCharacterActivity : BaseActivity<ActivityCustomizeBinding>() {
    ...
    override fun initView() {
        ...
        binding.btnResetMove.isSelected=true
            ...
            launch {
                viewModel.bottomNavigationList.collect { bottomNavigationList ->
                    if (bottomNavigationList.isNotEmpty()) {
                        ...
                        val navLayerIndex =
                            if (selectedNavIndex >= 0) bottomNavigationList[selectedNavIndex].layerIndex
    ...
    override fun viewListener() {
        binding.apply {
            actionBar.apply {
                ...
                actionBar.btnActionBarRightText.tap(1000) {
    ...
    private fun openMovePanel() {
        binding.layoutMove.visible()
        binding.rcvLayer.invisible()
        binding.flBottomNav.invisible()
        binding.btnMove.setImageResource(R.drawable.ic_move_slt)
        refreshMoveControls()
    }
    ...
    private fun scaleSelectedLayer(delta: Float) {
        ...
        setSelectedLayerScale(scale)
    }

    private fun setSelectedLayerScale(scale: Float) {
        val imageView = selectedLayerView() ?: return
        val transform = selectedLayerTransform() ?: return
    ...
    private fun refreshMoveControls() {
        ...
        updateScaleThumbPosition(imageView.scaleX)
        updateResetBtn()
    }

    @SuppressLint("ClickableViewAccessibility")
    private fun initScaleThumbTouch() {
        binding.icThumb1.setOnTouchListener { _, event ->
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN, MotionEvent.ACTION_MOVE -> {
                    setSelectedLayerScale(scaleFromRawY(event.rawY))
                    true
                }
    ...
    private fun updateScaleThumbPosition(scale: Float) {
        if (isFinishing || isDestroyed) return
        val frameScale = binding.frameScale
        val thumb = binding.icThumb1
        if (frameScale.height == 0) {
            frameScale.post {
                if (!isFinishing && !isDestroyed) {
                    updateScaleThumbPosition(scale)
                }
            }
            return
        }
        val trackTop = frameScale.y + thumb.height / 2f
        val trackBottom = frameScale.y + frameScale.height - thumb.height / 2f
        val scalePercent = (scale - MIN_SCALE) / (MAX_SCALE - MIN_SCALE)
        val targetCenterY = trackBottom - (trackBottom - trackTop) * scalePercent
        val baseCenterY = thumb.y - thumb.translationY + thumb.height / 2f
        thumb.translationY = targetCenterY - baseCenterY
    }

    private fun scaleFromRawY(rawY: Float): Float {
        val parentLocation = IntArray(2)
        (binding.frameScale.parent as View).getLocationOnScreen(parentLocation)
        val yInParent = rawY - parentLocation[1]
        val trackTop = binding.frameScale.y + binding.icThumb1.height / 2f
        val trackBottom = binding.frameScale.y + binding.frameScale.height - binding.icThumb1.height / 2f
        if (trackTop == trackBottom) return selectedLayerView()?.scaleX ?: 1f
        val clampedY = yInParent.coerceIn(trackTop, trackBottom)
        val scalePercent = (trackBottom - clampedY) / (trackBottom - trackTop)
        return MIN_SCALE + (MAX_SCALE - MIN_SCALE) * scalePercent
    }
                    ...
                    if (viewModel.statusFrom == ValueKey.CREATE) {
                        ...
                        setOf(manBodyIndex, womanBodyIndex)
                            ...
                            .forEach { bodyIdx ->
                                ...
                                if (viewModel.pathSelectedList.getOrNull(posCustom)
                                        .isNullOrEmpty()
                                ) {
                ...
                if (deferred1.await() && deferred2.await() && deferred3.await()) {
                    when (viewModel.statusFrom) {
                        ValueKey.CREATE -> {
                            ...
                            val manBodyIndex = layers.indexOfFirst { it.type == 1 || it.type == 0 }
                            val womanBodyIndex =
                                layers.indexOfFirst { it.type == 2 || it.type == 0 }
    ...
    private fun handleFillLayer(item: ItemNavCustomModel, position: Int) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                android.util.Log.d(
                    "FocusDebug",
                    "handleFillLayer submitList: positionNavSelected=${viewModel.positionNavSelected}, listSize=${listToSubmit.size}"
                )
                listToSubmit.forEachIndexed { i, it ->
                    android.util.Log.d(
                        "FocusDebug",
                        "  submit[$i] isSelected=${it.isSelected} path=${it.path}"
                    )
                }
    ...
    private fun handleClickBottomNavigation(positionBottomNavigation: Int) {
        val layerIndex =
            viewModel.bottomNavigationList.value.getOrNull(positionBottomNavigation)?.layerIndex
                ?: positionBottomNavigation
    ...
    private fun handleGenderSwitch(gender: Int) {
        ...
        android.util.Log.d(
            "GenderSwitch",
            "=== handleGenderSwitch: ${viewModel.selectedGender.value} -> $gender ==="
        )
        // Lưu nav đang chọn của gender hiện tại trước khi switch
        viewModel.genderNavPositionMap[viewModel.selectedGender.value] =
            viewModel.positionNavSelected
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            android.util.Log.d(
                "GenderSwitch",
                "positionNavSelected=${viewModel.positionNavSelected}, positionCustom=${viewModel.positionCustom}"
            )
            ...
            if (currentPath.isNullOrEmpty()) {
                val selectedItem = listToSubmit.firstOrNull {
                    it.isSelected &&
                            it.path != com.skin.rbx.clothes.makek.core.utils.key.AssetsKey.NONE_LAYER &&
                            it.path != com.skin.rbx.clothes.makek.core.utils.key.AssetsKey.RANDOM_LAYER
                }
