...
import com.skin.rbx.clothes.makek.core.custom.text.OuterStrokeTextView
...
import com.skin.rbx.clothes.makek.databinding.ActivityAddCharacterBinding
import com.skin.rbx.clothes.makek.dialog.ChooseColorDialog
import com.skin.rbx.clothes.makek.dialog.DialogSpeech
...
import com.skin.rbx.clothes.makek.ui.add_character.adapter.AddCharacterCategoryAdapter
import com.skin.rbx.clothes.makek.ui.add_character.adapter.BackgroundColorAdapter
import com.skin.rbx.clothes.makek.ui.add_character.adapter.BackgroundImageAdapter
import com.skin.rbx.clothes.makek.ui.add_character.adapter.SpeechAdapter
import com.skin.rbx.clothes.makek.ui.add_character.adapter.StickerAdapter
import com.skin.rbx.clothes.makek.ui.add_character.adapter.TextColorAdapter
import com.skin.rbx.clothes.makek.ui.add_character.adapter.TextFontAdapter
...
import com.skin.rbx.clothes.makek.ui.success.SuccessActivity
...
class AddCharacterActivity : BaseActivity<ActivityAddCharacterBinding>() {
    ...
    private val stickerCategoryAdapter by lazy{
        AddCharacterCategoryAdapter()
    }

    private val speechCategoryAdapter by lazy{
        AddCharacterCategoryAdapter()
    }
...
//                        isFocusEditText
                        viewModel.isFocusEditText.collect { status ->
                            ...
                            if (status) {
                                ...
                                viewModel.layoutParams.topMargin =
                                    UnitHelper.dpToPxInt(resources, -160f)
    ...
    override fun viewListener() {
        binding.apply {
            ...
            btnSticker.tap { checkInternet {
                lifecycleScope.launch {
                    withContext(Dispatchers.IO){
                        viewModel.reloadStickerData(this@AddCharacterActivity)
                    }
            ...
            btnSpeech.tap { checkInternet {
                lifecycleScope.launch {
                    withContext(Dispatchers.IO){
                        viewModel.reloadSpeechData(this@AddCharacterActivity)
                    }
            ...
            stickerCategoryAdapter.onCategoryClick = {position ->
                ...
                viewModel.stickerCategoryList.getOrNull(position)?.let{
                    category ->
                    stickerAdapter.submitList(category.items)
                }
              //  stickerAdapter.submitList(viewModel.stickerCategoryList[position].items)
            }
            ...
            speechCategoryAdapter.onCategoryClick = {position ->
                ...
                viewModel.speechCategoryList.getOrNull(position)?.let {
                    category ->
                    speechAdapter.submitList(category.items)
                }
               // speechAdapter.submitList(viewModel.speechCategoryList[position].items)
            }

            speechAdapter.onItemClick = { path -> checkInternet {  handleSpeech(path) }}
    ...
    private fun initRcv() {
        binding.apply {
            ...
            speechTab.rcvFont.apply {
                adapter = textFontAdapter
                itemAnimator = null
            }

            speechTab.rcvTextColor.apply{
    ...
    private fun initData() {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                binding.textTab.edtText.setFont(viewModel.textFontList.first().value)
                binding.textTab.edtText.setTextColor(viewModel.textColorList[1].value)
                binding.tvGetText.setFont(viewModel.textFontList.first().value)
                binding.tvGetText.setTextColor(viewModel.textColorList[1].value)
...
//                binding.drawView.autoSelectFirstDraw()
    ...
    private fun handleChooseColor(isTextColor: Boolean = false) {
        ...
        dialog.onDoneEvent = { color ->
            ...
            if (!isTextColor) {
                ...
                handleSetBackgroundColor(color, BackgroundColorAdapter.ADD_COLOR_POSITION)
            } else {
    ...
    private fun handleSpeech(path: String) {
        ...
        val selectedFont = viewModel.textFontList.firstOrNull(){
            it.isSelected}?.value
            ?:viewModel.textFontList[1].value

        val selectedColor = viewModel.textColorList.firstOrNull(){
            it.isSelected }?.value
            ?: viewModel.textColorList[1].value

        val dialog = DialogSpeech(this, path,selectedFont,selectedColor)
        dialog.show()
    ...
    private fun handleSetBackgroundColor(color: Int, position: Int) {
        ...
        binding.apply {
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                Log.d(
                    "AddCharacterActivity",
                    "Before updateBackgroundColorSelected: list[0].value=${
                        String.format(
                            "#%06X",
                            0xFFFFFF and viewModel.backgroundColorList[0].value
                        )
                    }"
                )
                ...
                Log.d(
                    "AddCharacterActivity",
                    "After updateBackgroundColorSelected: list[0].value=${
                        String.format(
                            "#%06X",
                            0xFFFFFF and viewModel.backgroundColorList[0].value
                        )
    ...
    private fun handleTextColorClick(color: Int, position: Int) {
        ...
        binding.apply {
            ...
            Log.d(
                "AddCharacterActivity",
                "Before updateTextColorSelected: list[0].value=${
                    String.format(
                        "#%06X",
                        0xFFFFFF and viewModel.textColorList[0].value
                    )
                }"
            )
            ...
            Log.d(
                "AddCharacterActivity",
                "After updateTextColorSelected: list[0].value=${
                    String.format(
                        "#%06X",
                        0xFFFFFF and viewModel.textColorList[0].value
                    )
    ...
    private fun handleDoneText() {
        ...
        binding.apply {
            if (textTab.edtText.text.toString().trim() == "") {
                ...
                // Reset
                val font = viewModel.textFontList.first().value
                val color = viewModel.textColorList[1].value
