...
import com.skin.rbx.clothes.makek.databinding.FragmentMyAvatarBinding
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    viewModel.myAvatarList.collect { list ->
                        ...
                        myAlbumActivity.updateAvatarEmptyState(list.isEmpty())
                    }
                }
    ...
    override fun viewListener() {
        binding.apply {
            ...
            myAvatarAdapter.onItemClick = { pathInternal -> handleItemClick(pathInternal)
                Log.d("ViewPath","click path = $pathInternal modified = ${File(pathInternal).lastModified()} size= ${File(pathInternal).length()})}")
