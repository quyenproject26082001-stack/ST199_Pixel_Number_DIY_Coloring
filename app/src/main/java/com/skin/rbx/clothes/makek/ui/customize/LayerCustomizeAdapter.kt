...
import com.skin.rbx.clothes.makek.databinding.ItemCustomizeBinding
import com.bumptech.glide.Glide

class LayerCustomizeAdapter(val context: Context) :
    ListAdapter<ItemNavCustomModel, LayerCustomizeAdapter.CustomizeViewHolder>(DiffCallback) {

    var onItemClick: ((ItemNavCustomModel, Int) -> Unit) = { _, _ -> }
    var onNoneClick: ((Int) -> Unit) = {}
    var onRandomClick: (() -> Unit) = {}

    inner class CustomizeViewHolder(val binding: ItemCustomizeBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(item: ItemNavCustomModel, position: Int) {
            binding.apply {
                val cornerRadiusPx = UnitHelper.dpToPxInt(context.resources, 4f)
                ...
                if (item.isSelected) {
                    ...
                    cardLayerItem.setStrokeColor(Color.BLACK)
                    ...
                    root.scaleY = 1f
                    vFocus.gone()
                    cardLayerItem.setStrokeColor(Color.TRANSPARENT)
    ...
    companion object {
        val DiffCallback = object : DiffUtil.ItemCallback<ItemNavCustomModel>() {
            override fun areItemsTheSame(
                oldItem: ItemNavCustomModel,
                newItem: ItemNavCustomModel
            ): Boolean {
